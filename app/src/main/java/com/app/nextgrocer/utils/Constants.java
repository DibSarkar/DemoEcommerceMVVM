package com.app.nextgrocer.utils;

public class Constants {

    public static final String BASE_URL = "http://192.168.5.51/dheman/index.php?route=appapi/";
   // public static final String BASE_URL = "http://developersatwork.com/projects/f7/dheman/index.php?route=appapi/";
    public static final String API_TOKEN = "64a355912c41857b24762623e3";
    public static final int SPACE_HOME_FRAGMENT = 1;
    public static final int SPACE_CATEGORIES_FRAGMENT= 2;
    public static final int SPACE_PRODUCT_LIST= 3;
    public static final int SPACE_CHECKOUT_LIST= 4;
    public static final int SPACE_HOME_CATYEGORY_LIST= 5;

    public static final String FEATURED_PRODUCTS = "Featured Products";
    public static final String NEWARRIVAL_PRODUCTS = "New Arrival Products";
    public static final String CATEGORY_PRODUCTS = "Category Products";
    public static final String SORT_NAME = "pd.name";
    public static final String SORT_PRICE = "p.price";
    public static final String SORT_RATING = "rating";

    public static final String SORT_ASC = "ASC";
    public static final String SORT_DESC = "DESC";



    public static final String DB_NAME = "mindorks_mvvm.db";



    public static final String PREF_NAME = "mindorks_pref";


}
